...
- [ ] 1.1 `DigestActiveWorkSchema`, `DigestHeadsUpSchema` (+ `DigestHeadsUpKind`), `DigestTalliesSchema`
...
- [ ] 1.2 Contract test: a fully-populated digest parses; the new fields are required (a digest
      missing them is rejected); `headsUp.kind` is restricted to `danger|warning`.

## 2. core — projection (TDD red→green)

- [ ] 2.1 `projectDigest`: stamp `risk`/`reversible`/`isNew` on each `waiting` row (`isNew` from the
      set of asks parked in the window).
- [ ] 2.2 `projectDigest`: derive `activeWork` (task `ACTIVE` & not blocked-on-ask; carry the parent
...
- [ ] 2.3 Unit tests: `isNew` boundary at the cursor; risk/reversible passthrough; `activeWork`
...
- [ ] 3.1 Server: confirm the REST digest endpoint returns the new fields (passthrough); add/extend
      a route test asserting `activeWork`/`headsUp`/`tallies` are present.
- [ ] 3.2 Web: extend mock fixtures (`MOCK_DIGEST` / `fixtures.ts`) and any inline digests to the
