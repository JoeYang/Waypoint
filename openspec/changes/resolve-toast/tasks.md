...
- [ ] 1.1 `ToastProvider` + `useToast()` hook: a context holding a queue of `{ id, message }`,
...
- [ ] 1.2 `ToastProvider.module.css` on the Axiom tokens (fixed bottom region, raised surface).
- [ ] 1.3 Mount `<ToastProvider>` wrapping the app tree (`main.tsx`).
- [ ] 1.4 Tests: `toast(msg)` renders a `role="status"` with the message; advancing fake timers
