...
- [ ] 1.1 Add `MissionControl.module.css` (axiom tokens, full-screen takeover, top bar, 3-column
  grid, stream progress bar, danger/warning heads-up, footer stat strip).
- [ ] 1.2 Write `MissionControl.test.tsx` (red): ready renders a needs-you DecisionCard, an
  ...
  render.
- [ ] 1.3 Implement `MissionControl` ({ projectId, onClose }) over `useReentry` + `useWaypoint`
  (for per-stream progress). Green.

## 2. Verify
- [ ] 2.1 `npm test`, `npx tsc -b`, `npx eslint .` (own files clean), `npx prettier --write`.
- [ ] 2.2 `npx openspec validate reentry-mission-control --strict`.
