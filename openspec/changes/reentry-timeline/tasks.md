...
- [ ] 1.1 Extend `useReentry.test.ts` (red): assert the ready model exposes a populated `timeline`
      (the story entries, oldest-first) and `sinceSeq` (the digest cursor). Keep the existing
      loading/error assertions green.
- [ ] 1.2 Add `timeline: StoryEntry[]` and `sinceSeq: number` to `ReentryModel`, sourced from the
      already-fetched story response and the digest. Loading/error/ready contract unchanged. Green.

## 2. Timeline drawer surface

- [ ] 2.1 Add `TimelineDrawer.module.css` (axiom tokens, right-side drawer, pinned needs-you header,
      session-replay list with time/label/node/actor rows, "new since you left" divider, footer).
- [ ] 2.2 Write `TimelineDrawer.test.tsx` (red): ready renders a needs-you DecisionCard, a
...
- [ ] 2.3 Implement `TimelineDrawer` ({ projectId, onClose }) over `useReentry` + `useWaypoint`.
      Green.

## 3. Verify

- [ ] 3.1 `npm test`, `npx tsc -b`, `npx eslint .` (own files clean), `npx prettier --write`.
- [ ] 3.2 `npx openspec validate reentry-timeline --strict`.
